...
export function createWhatsAppMarketingMcpServer(bridge, config) {
    const server = new McpServer({
        name: "cuiabar-whatsapp-marketing",
        version: "0.1.0"
    });
    server.registerTool("create_message_variants", {
    ...
    server.registerTool("plan_consented_campaign", {
        title: "Planejar campanha consentida",
        description: "Monta um plano de disparo com janela, limites e cadencia. Em validateOnly/trainingMode nao exige consentimento no payload.",
        inputSchema: campaignPlanSchema()
    }, async (input) => textResult(buildCampaignPlan(input, config)));
    server.registerTool("format_whatsapp_marketing_message", {
    ...
    server.registerTool("send_form", {
        title: "Enviar form",
        description: "Envia um form de texto com opcoes 1, 2, 3... O cliente responde somente com o numero, o bridge responde uma vez e encerra a sessao.",
        inputSchema: {
    ...
    server.registerTool("send_numbered_menu", {
    ...
    server.registerTool("send_single_marketing_message", {
    ...
    server.registerTool("send_single_marketing_media", {
        title: "Enviar midia consentida individual",
        description: "Simula ou envia foto, video, audio, mensagem de voz ou arquivo. Em validateOnly/trainingMode nao exige consentimento, opt-out ou remetente no texto.",
        inputSchema: {
            recipient: recipientSchema(),
    ...
    server.registerResource("policy", "whatsapp-marketing-mcp://policy", {
