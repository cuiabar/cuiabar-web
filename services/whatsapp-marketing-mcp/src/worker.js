...
import { buildCampaignPlan, buildMessageVariants, formatWhatsAppMarketingMessage, sendNumberedMenu, sendSingleMarketingMedia, sendSingleMarketingMessage } from "./policy.js";
const REQUIRED_ENV = ["MCP_BEARER_TOKEN", "GHCO_COMMS_BRIDGE_TOKEN"];
...
async function handleActionRequest(request, env) {
    const authError = await requireBearerToken(request, env);
    if (authError) {
        return authError;
    }
    ...
    if (url.pathname === "/actions/message-variants" && request.method === "POST") {
        return json(buildMessageVariants(await readJson(request)));
    }
    if (url.pathname === "/actions/plan-campaign" && request.method === "POST") {
        return json(buildCampaignPlan(await readJson(request), config));
    }
    if (url.pathname === "/actions/format-message" && request.method === "POST") {
        return json(formatWhatsAppMarketingMessage(await readJson(request)));
    }
    if (url.pathname === "/actions/send-single" && request.method === "POST") {
        return json(await sendSingleMarketingMessage(bridge, await readJson(request)));
    }
    if ((url.pathname === "/actions/send-form" || url.pathname === "/actions/send-numbered-menu") && request.method === "POST") {
        return json(await sendNumberedMenu(bridge, await readJson(request)));
    }
    if (url.pathname === "/actions/send-media" && request.method === "POST") {
        return json(await sendSingleMarketingMedia(bridge, await readJson(request)));
    }
    return json({ error: "Not found" }, 404);
}
function createBridgeClient(env) {
...
function openApiSchema(origin) {
    ...
    return {
        ...
        info: {
            ...
            version: "0.1.0",
            description: "GPT Actions API para treinamento e campanhas WhatsApp. Em validateOnly/trainingMode relaxa consentimento e opt-out no payload; envio real continua exigindo confirmacao e validacao."
        },
        servers: [{ url: origin }],
        components: {
            securitySchemes: {
...
async function requireBearerToken(request, env) {
...
function authorizationPage(url) {
    const hiddenFields = [
        "client_id",
        "redirect_uri",
        "response_type",
...
async function issueTokens(env, scope = `${readScope} ${writeScope}`) {
...
async function verifyPayload(token, env) {
...
async function verifyPkce(verifier, challenge, method = "plain") {
    if (!challenge) {
        return true;
    }
    if (method === "S256") {
