...
export type BridgeClient = {
  sendText(recipient: string, text: string): Promise<unknown>;
  sendMedia(input: {
    recipient: string;
    mediaType: MediaType;
    filePath?: string;
...
type VariantInput = {
  ...
  audience: string;
  offer?: string;
  tone: "direto" | "cordial" | "premium" | "urgente_suave";
  variantCount: number;
  optOutText: string;
};
...
type SendSingleInput = {
  ...
  validateOnly?: boolean;
  trainingMode?: boolean;
  confirmWrite?: "ENVIAR_WHATSAPP_CONSENTIDO";
};
...
type SendMediaInput = {
  ...
  validateOnly?: boolean;
  trainingMode?: boolean;
  confirmWrite?: "ENVIAR_WHATSAPP_MIDIA_CONSENTIDA";
};

type SendNumberedMenuInput = {
  recipient: Recipient;
  title?: string;
  body: string;
  options: NumberedMenuOption[];
  ...
  validateOnly?: boolean;
  trainingMode?: boolean;
  confirmWrite?: "ENVIAR_MENU_NUMERADO";
};
...
export function buildMessageVariants(input: VariantInput): Record<string, unknown> {
  ...
  const styles: Record<VariantInput["tone"], string[]> = {
...
export function buildCampaignPlan(input: CampaignPlanInput, config: MarketingConfig): Record<string, unknown> {
...
export async function sendSingleMarketingMessage(bridge: BridgeClient, input: SendSingleInput): Promise<Record<string, unknown>> {
...
export function formatWhatsAppMarketingMessage(input: FormatMessageInput): Record<string, unknown> {
...
export async function sendSingleMarketingMedia(bridge: BridgeClient, input: SendMediaInput): Promise<Record<string, unknown>> {
  const trainingOrValidation = isTrainingOrValidation(input);
  const errors = validateRecipient(input.recipient, { requireConsent: !trainingOrValidation, honorOptOut: !trainingOrValidation });
...
export async function sendNumberedMenu(bridge: BridgeClient, input: SendNumberedMenuInput): Promise<Record<string, unknown>> {
  const trainingOrValidation = isTrainingOrValidation(input);
  const errors = [
...
function validateCampaignInput(input: CampaignPlanInput, config: MarketingConfig): CampaignValidation {
  const errors: string[] = [];
  ...
  for (const [index, recipient] of input.recipients.entries()) {
    validateRecipient(recipient, { requireConsent: !trainingOrValidation, honorOptOut: !trainingOrValidation }).forEach((error) =>
...
function validateRecipient(recipient: Recipient, options: { requireConsent: boolean; honorOptOut: boolean }): string[] {
  const errors: string[] = [];
...
function validateMarketingText(text: string): string[] {
  const errors: string[] = [];
...
function validateMediaInput(input: SendMediaInput): string[] {
  const errors: string[] = [];
...
function validateNumberedMenu(input: SendNumberedMenuInput): string[] {
  const errors: string[] = [];
...
function buildFormattedText(input: FormatMessageInput): string {
...
function formatNumberedMenu(input: Pick<SendNumberedMenuInput, "title" | "body" | "options" | "footer">): string {
  const sections: string[] = [];
  const title = cleanLine(input.title);
  const body = cleanBlock(input.body);
  const footer = cleanBlock(input.footer);
...
function isTrainingOrValidation(input: { validateOnly?: boolean; trainingMode?: boolean }): boolean {
...
function compliancePolicy(input?: { validateOnly?: boolean; trainingMode?: boolean }): Record<string, unknown> {
